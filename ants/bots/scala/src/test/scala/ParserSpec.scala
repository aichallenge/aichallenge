...
class ParserSpec extends Specification with ScalaCheck {
  val gameParameterGenerator = for {
    ...
    seed <- Gen.oneOf(1 to 500)
    viewRadius <- Gen.oneOf(1 to 200)
    attackRadius <- Gen.oneOf(1 to 200)
    spawnRadius <- Gen.oneOf(1 to 200)
  } yield(GameParameters(loadTime, turnTime, rows, columns, turns, seed, viewRadius, attackRadius, spawnRadius))
    ...
    "parse valid turn zero data" ! checkProp {
      ...
      (params: GameParameters) =>
        val input = """turn 0
          ...
          attackradius2 %d
          spawnradius2 %d
          ready""".format(params.loadTime, params.turnTime, params.rows, params.columns, params.turns, params.seed,
