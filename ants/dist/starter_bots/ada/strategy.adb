...
--
--   Copyright 2011 Phil Costin / philxyz
--
...
package body Strategy is
   ...
   begin
      ...
      -- Attempt to go in the following directions in this order:
      --    N, E, S, W

      declare
         Bearings : array(1 .. 4) of Ants.Bearing :=
           (Ants.North, Ants.East, Ants.South, Ants.West);

         Sq : Ants.Square;
      begin
         for New_Bearing in Bearings'Range loop
            -- Get the Square identified by the next Bearing in the list
            Sq := Ants.Examine_Square (My_Position, Bearings (New_Bearing));

            -- If the square is passable and unoccpied, move into the square
            if Sq.Kind in Ants.Passable_Tile and then Sq.Kind in Ants.Unoccupied_Tile then
               Ants.Issue_Order (My_Position, Bearings (New_Bearing));
               exit;
            end if;
