...
def analyze_map(map_location):
	...
	while len(square_queue) > 0:
		...
		for d in directions:
			...
			if map[nLoc[0] ][nLoc[1] ] != '%':
				if access_map[nLoc[0] ][nLoc[1] ][0] == -1: #first time square has been reached
	...
	land_access_players = defaultdict(int)

	for row in access_map:
		for cell in row:
			t = cell[1]
			if len(t) > 1:
				land_access_players[tuple(t)] += 1
			elif len(t) == 1:
				land_access_players[t[0]] += 1
