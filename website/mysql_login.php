...
$my_escape = function($arg) {
    return mysqli_real_escape_string($mysqli, $arg);
};

function contest_query() {
    global $contest_sql;
    $args = func_get_args();
    $mysqli = array_shift($args);
    if (count($args) >= 1) {
        $query_name = $args[0];
        if (count($args) > 1) {
            $query_args = array_map($my_escape, array_slice($args, 1));
...
function check_credentials($mysqli, $username, $password) {
  $query = "
        SELECT *
        FROM user u
        WHERE
...
function check_credentials_forgot($mysqli, $user_id, $forgot_code) {
    // $login_cookie is not encrypted nor stored in the database
    // $user_cookie['cookie'] is encrypted
    $user_forgets = contest_query($mysqli, "select_user_forgot_code", $user_id);
    while ($user = mysqli_fetch_assoc($user_forgets)) {
        if (crypt($forgot_code, $user['cookie']) == $user['cookie']) {
            ...
            contest_query($mysqli, "delete_user_cookie", $user_id, $user['cookie']);     
            // update session vars
            $_SESSION['username']   = $user['username'];
            $_SESSION['admin']      = $user['admin'];
            $_SESSION['user_id']    = $user['user_id'];
 ...
 */
function check_credentials_cookie($mysqli, $user_id, $login_cookie) {
    // $login_cookie is not encrypted nor stored in the database
    // $user_cookie['cookie'] is encrypted
    $user_cookies = contest_query($mysqli, "select_user_cookies", $user_id);
    while ($user = mysqli_fetch_assoc($user_cookies)) {
        if (crypt($login_cookie, $user['cookie']) == $user['cookie']) {
            // found valid cookie, reset expire date
            contest_query($mysqli, "update_user_cookie", $user_id, $user['cookie']);
 ...
 * @since 28 Oct 2011 bear@deepshiftlabs.com
 */
function create_user_cookie($mysqli, $user_id) {
    if (isset($_SESSION['user_id'])) {
        $user_id = $_SESSION['user_id'];
        $login_cookie = $user_id . "-" . salt(32, true);
        $encrytped_cookie = crypt($login_cookie, '$6$rounds=54321$' . salt() . '$');
        if (contest_query($mysqli, "insert_user_cookie", $user_id, $encrytped_cookie)) {
...
function delete_user_cookie($mysqli) {
    if (isset($_SESSION['user_id']) && isset($_SESSION['cookie'])) {
        contest_query($mysqli, "delete_user_cookie", $_SESSION['user_id'], $_SESSION['cookie']);
    }
}

function create_user_forgot_code ($mysqli, $username) {
    $user_result = contest_query($mysqli, "select_user_by_name", $username);
    if ($user_result) {
        ...
        if (contest_query($mysqli, "insert_user_forgot_code", $user_id, $encrypted_code)) {
